...
8比特子元素环境检索器代码（SubEES_8Bit_Code）通过命名空间定义系统用8比特子元素环境检索器操作代码。通过该代码赋值，以此完成对系统用8比特子元素环境检索器的各种元素完成操作。

关于如何在代码中使用该命名空间，跳转至[8比特子检索器测试案例](../EES_Manage_Case/8Bit_Sub_EES_Test_Case.md)。
