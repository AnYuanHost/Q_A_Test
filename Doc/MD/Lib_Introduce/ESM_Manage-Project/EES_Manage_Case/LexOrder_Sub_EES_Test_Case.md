...
本案例展示字典序子检索器如何在总元素环境检索器中完成工作，并提供测试案例数据展示如何进行测试。

字典序子检索器主要用于适合使用字典序流进行检索的场景，例如直接面向语言文字的语境。使用字典序作为检索，便可以通过语言追踪元素。
...
本案例需要使用三个库，分别是E_S_M库提供基本操作，以及ESM_Manage库提供字典序子检索器服务，中间计算需要使用Math库。因为呈现测试数据需要使用std::cout进行输出，所以还需要引入iostream。
...
在完成中间变量的构造后，调用参数构造映射和构造映射完成字典序子检索器的构造工作。
...
按照"Name00" ~ "Name99"代表的超索引信息与0~99代表的超索引信息间的映射完成注册。
...
为了方便后续的排序，碎片整理和缩容，在这里部分注销。按照隔一个注销一个的策略，将"Name00"，"Name02"..."Name98"对应的映射进行注销。
...
在完成演示后，注销剩余的元素，并完成字典序子检索器的排序、碎片整理与缩容。这会释放掉所有槽位，避免在释放子检索器时出现内存泄漏等问题。
