...
本案例展示8比特子检索器如何在总元素环境检索器中完成工作，并提供测试案例数据展示如何进行测试。

因为8比特子检索器的设计以性能为主，所以在庞杂系统中作为向系统级的使用需求提供服务。8比特子检索器也具备定长等性质，这就要求在设计与实现方式上，和变长类容器间存在一定的差异。在本案例中涉及到这些差异的代码段，专门进行讲解。
...
本案例需要使用三个库，分别是E_S_M库提供基本操作，以及ESM_Manage库提供8比特子检索器服务，中间计算需要使用Math库。因为呈现测试数据需要使用std::cout进行输出，所以还需要引入iostream。
...
在完成中间变量的构造后，直接调用构造映射完成8比特子检索器的构造工作。
...
按照1~100代表的8比特信息与0~99代表的超索引信息间的映射完成注册。
...
为了方便后续的排序，碎片整理和缩容，在这里部分注销。按照隔一个注销一个的策略，将1、3、5...99对应的映射进行注销。
...
在完成隔一注销后，置懒和非置懒的槽位都被分割开。这里演示8比特子检索器的下一地址是如何完成置懒槽位和非置懒槽位的不同计算。显然，8比特子检索器计算的下一地址是区分槽位是否被置懒的。
...
同时，也可以看到，在碎片整理的步骤中，因为8比特子检索器是定长容器，且其检索信息和地址信息高度绑定，完全不需要进行整理，所以这里调用空映射。而在面向总元素检索器的注册时，也会使用空映射填补碎片整理映射的槽位。
...
在完成演示后，注销剩余的元素，并完成8比特的子检索器的排序、碎片整理与缩容。这会释放掉所有槽位，避免在释放子检索器时出现内存泄漏等问题。
